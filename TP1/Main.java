...
class Main {
        ...
        while(sair == 0) {
            ...
            switch (escolha) {
                ...
                case 3:
                    ...
                    //Mesmo metodo de busca do READ
                    while (g_temp.getID() != lastID && achou == false) {
                        //Salva a posiçao do id encontrado
                        pos = arq.getFilePointer();
                        int flag = arq.readInt();
                        if (flag == 1) {
                            len = arq.readInt();
                    ...
                    if (achou == true) {
                        ...
                        System.out.println("Digite a nova data de lançamento do jogo no formato: YYYY-MM-DD, Ex: 2008-11-05");
                        ...
                        System.out.println("Digite o(s) desenvolvedor(es) do jogo usando separaçao por ','. Ex: Dev1, Dev2, Dev3");
                        newDeveloper = MyIO.readLine().split(",");
    
                        //Scanner para a mudança de publicadores, utilizando "," como separador
                        System.out.println("Digite o(s) publicador(es) do jogo usando separaçao por ','. Ex: Pub1, Pub2, Pub3");
                        newPublisher = MyIO.readLine().split(",");
    
                        //Scanner para a mudança de plataformas, utilizando "," como separador
                        System.out.println("Digite a(s) plataforma(s) do jogo usando separaçao por ','. Ex: windows, mac, linux");
                        newPlatforms = MyIO.readLine().split(",");
    
                        //Scanner para a mudança de categorias, utilizando "," como separador
                        System.out.println("Digite a(s) categoria(s) do jogo usando separaçao por ','. Ex: Online, Multiplayer local, Valve Anticheat enabled");
                        newCategories = MyIO.readLine().split(",");
    
                        //Scanner para a mudança de generos, utilizando "," como separador
                        System.out.println("Digite o(s) genero(s) do jogo usando separaçao por ','. Ex: Action, Adventure, Indie");
                        newGenres = MyIO.readLine().split(",");

                        //Criaçao de um novo game com os dados recebidos
                        Game newGame = new Game(id, newLan_support, newRequired_age, newName, newDate, newDeveloper, newPublisher, newPlatforms, newCategories, newGenres);
                        byte[] newBa;
                        int newLen;
                        ...
                        if (newLen == len) {
                            //Caso o registro seja igual ao antigo, setar no mesmo local, sobrepondo os dados
                            arq.seek(pos);
                            arq.writeInt(1); //Lapide registrando como valido
                            arq.writeInt(len); //Tamanho do registro em bytes
                            arq.write(newBa);
                        } else {
                            //Caso o registro seja maior ou menor que o antigo, mover para o final do arquivo e "apagar" o outro
                            arq.seek(pos);
                ...
                case 4:
                    ...
                    if (achou == true) {
                        ...
                        //Caso o registro deletado fosse o ultimo, mudar o cabeçalho para o penultimo ID (newLastID)
                        if (id == lastID) {
    ...
    public static void lerArquivoCsv() throws Exception {
        ...
        while (sc.hasNextLine()) {
          ...
          game[i] = new Game(id, lan_support, required_age, name, date, developer, publisher, platforms, categories, genres);
          i++;
        }
        sc.close(); 
        ...
        if (separado.length == 3) {
            try {
                ...
                if (ano >= 1000 && ano <= 2023) { //Verifica se o ano é valido
                    try {
                        Integer.parseInt(separado[1]); //Verifica se é inteiro, caso contrario, entra no catch e retorna false
                        int mes = Integer.valueOf(separado[1]);
                        if (mes >=1 && mes <= 12) { //Verifica se o mes é valido
                            try {
                                Integer.parseInt(separado[2]); //Verifica se é inteiro, caso contrario, entra no catch e retorna false
    ...
    public static int intercala(String arquivoEntrada, int numIntercala) throws Exception {
        ...
        for (int i = 0; i < numIntercala; i++) {
            ...
            for (int j = 0; j < pow; j++) {
                if ((int) Math.ceil((double)cont1/pow) == (int) Math.ceil((double)n/pow)) { //Caso o valor seja arredondado
