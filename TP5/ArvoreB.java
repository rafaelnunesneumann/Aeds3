...
public class ArvoreB {
    ...
    public void inserir(int k, long pos) {
        ...
        if (buscaChave(raiz, k) == null) { //só insere se não houver, para evitar duplicação de chaves
            ...
            if (raiz.getN() == 0) {
                ...
                if (r.getN() == ordem - 1) {//há necessidade de dividir a raiz
                    ...
                    insereNoNaoCheio(s, k, pos);//depois de dividir a raiz começa inserindo apartir da raiz

                } else {//caso contrario começa inserindo apartir da raiz
                    insereNoNaoCheio(r, k, pos);
                }
            }
    ...
    public long buscarPos(int chave) {
        No no = buscaChave(raiz, chave);
        long pos = 0;
        if (no != null) {
            for (int i = 0; i < no.getN(); i++) {
                if (no.getChave().get(i) == chave) {
                    pos = no.getPos().get(i);
                    i = no.getN();
                }
            }
    ...
    public void divideNo(No x, int i, No y) {
        ...
        for (int j = 0; j < t; j++) {
            if ((ordem - 1) % 2 == 0) {
                ...
                z.getPos().set(j, y.getPos().get(j + t));
            } else {
                z.getChave().set(j, y.getChave().get(j + t + 1));
                z.getPos().set(j, y.getPos().get(j + t + 1));
            }
            y.setN(y.getN() - 1);
        }
    ...
    public void insereNoNaoCheio(No x, int k, long pos) {
        int i = x.getN() - 1;
        ...
        if (x.isFolha()) {
            ...
            while (i >= 0 && k < x.getChave().get(i)) {
                ...
                x.getPos().set(i + 1, x.getPos().get(i));
                i--;
            }
            i++;
            ...
            insereNoNaoCheio(x.getFilho().get(i), k, pos);
        }
        ...
        if (buscaChave(this.raiz, k) != null) {
            ...
            if (N.isFolha()) {
                for (int j = i + 1; j <= N.getN(); j++) {
                    ...
                    N.getPos().set(j - 2, N.getPos().get(j - 1));
                }
                N.setN(N.getN() - 1);
                if (N != this.raiz) {
                ...
                long p = S.getPos().get(S.getN() - 1);;//y é o antecessor de k
                S.setN(S.getN() - 1);
    ...
    private void Balanceia_Dir_Esq(No P, int e, No Dir, No Esq) {
        ...
        Esq.getPos().set(Esq.getN() - 1, P.getPos().get(e));
        P.getChave().set(e, Dir.getChave().get(0));//"sobe" uma chave de Dir para P
        P.getPos().set(e, Dir.getPos().get(0));
        ...
        for (int j = 1; j < Dir.getN(); j++) {
            ...
            Dir.getPos().set(j - 1, Dir.getPos().get(j));
        }
    ...
    private void Juncao_No(No X, int i) {
        ...
        for (int j = i; j <= X.getN() - 1; j++) {//ainda nao
            ...
            X.getPos().set(j - 1, X.getPos().get(j));
