...
class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (sair == 0) {
            ...
            System.out.println("| 8 - Busca por padrao                           |");
            System.out.println("| 9 - Criptografar                               |");
            System.out.println("| 10 - Descriptografar                           |");
            ...
            switch (escolha) {
                ...
                case 8:
                    transformarArquivo();
                    System.out.println("Digite o padrao a ser procurado:");
                    String padrao = MyIO.readLine();
                    pesquisarForcaBruta(padrao);
                    pesquisarKMP(padrao);
                    new File("temp.txt").delete();
                    break;
                case 9:
                    criptografar(chaveDES);
                    break;
                case 10:
                    descriptografar(chaveDES);
                    break;
                case 11:
                    sair = 1;
                    break;
    ...
    public static void lerArquivoCsv(ArvoreB arvore, Hashing hash, HashMap<String, ArrayList<Integer>> nomes,
            HashMap<String, ArrayList<Integer>> datas) throws Exception {
    ...
    public static void transformarArquivo() throws Exception {
        /*
         * Transforma o arquivo de bytes em um arquivo temporario mais "legivel"
         */
        try {
        ...
        while (g_temp.getID() != lastID) {
            ...
            if (flag == 1) {
                ...
                // Movendo os dados do vetor de bytes para o objeto
                g_temp.fromByteArray(ba);
                String data = dateToString(g_temp.getDate());
                writer.write("\n" + g_temp.getID() + "," + g_temp.getName() + "," + data + "," + g_temp.getLanSupport()
                        + "," + g_temp.getDeveloper() + "," + g_temp.getPublisher() + ","
                        + g_temp.getPlatforms() + "," + g_temp.getRequiredAge() + "," + g_temp.getCategories() + ","
                        + g_temp.getGenres());
            } else if (flag == 2) {
                // Caso o registro nao seja valido
                // Leitura do tamanho do registro
    ...
    public static void pesquisarForcaBruta(String padrao) throws Exception {
        ...
        for (int index = 0; index < max; index++) {
            for (int i = 0; i < padrao.length(); i++) {
                if (data.charAt(index + comp) == padrao.charAt(i)) {
                    numComp += 1;
                    comp++;
                    if (i == padrao.length() - 1) {
                        achou = true;
                        comp = 0;
                        index = max;
                    }
        ...
        if (achou == true) {
            System.out.println("Padrao encontrado! - Força Bruta");
            System.out.println("Numero de comparaçoes: " + numComp);
            System.out.println("Tempo utilizado: " + elapsed + " milissegundo(s)");
        } else {
            System.out.println("Padrao nao encontrado! - Força Bruta");
            System.out.println("Numero de comparaçoes: " + numComp);
            System.out.println("Tempo utilizado: " + elapsed + " milissegundo(s)");
    ...
    public static void pesquisarKMP(String padrao) throws FileNotFoundException {
        File myObj = new File("temp.txt");
        Scanner scan = new Scanner(myObj);
        String data = "";
        while (scan.hasNextLine()) {
        ...
        int[] next = new int[padrao.length() + 1];
        ...
        for (int i = 0, j = 0; i < data.length(); i++) {
            numComp++;
            if (j < padrao.length() && data.charAt(i) == padrao.charAt(j)) {
                if (++j == padrao.length()) {
                    achou = true;
                    i = data.length();
                }
        ...
        long elapsed = System.currentTimeMillis() - start;
        if (achou == true) {
            System.out.println("Padrao encontrado! - KMP");
            System.out.println("Numero de comparaçoes: " + numComp);
            System.out.println("Tempo utilizado: " + elapsed + " milissegundo(s)");
        } else {
            System.out.println("Padrao nao encontrado! - KMP");
            System.out.println("Numero de comparaçoes: " + numComp);
            System.out.println("Tempo utilizado: " + elapsed + " milissegundo(s)");
    ...
    public static String criptografar(SecretKey chaveDES) throws Exception {
        /*
         * Le a base de dados e salva os primeiros 100 IDS
         */
        RandomAccessFile arq = new RandomAccessFile("games.db", "rw");
        FileWriter writer = new FileWriter("cesar.txt");
        Game g_temp = new Game();
        byte[] ba;
        int len;
        int lastID = arq.readInt();
        ...
        for (int i = 0; i < 100; i++) {
            ...
            String data = dateToString(g_temp.getDate());
            /*
             * Adiciona todos as informações a um unico texto
             */
            texto += "," + g_temp.getID() + "," + g_temp.getName() + "," + data + "," + g_temp.getLanSupport() + ","
                    + g_temp.getDeveloper() + "," + g_temp.getPublisher() + "," + g_temp.getPlatforms() +
                    "," + g_temp.getRequiredAge() + "," + g_temp.getCategories() + "," + g_temp.getGenres();
        }
        /* Converte o texto e salva no arquivo */
        writer.write(converterCifra(texto));
        converterDES(texto, chaveDES);
        arq.close();
        writer.close();
        return texto;
    }

    public static void descriptografar(SecretKey chaveDES) throws Exception {
        /* Le o texto criptografado do arquivo */
        Scanner reader = new Scanner(new File("cesar.txt"));
        FileWriter writer = new FileWriter("cesarDescriptografado.txt");
        FileWriter writer2 = new FileWriter("desDescriptografado.txt");
        String texto = "";
        while (reader.hasNextLine()) {
            texto += reader.nextLine();
        }
        /* Descriptografa o texto e salva nos arquivos */
        writer.write(desconverterCifra(texto));
        writer2.write(desconverterDES(texto, chaveDES, converterDES(criptografar(chaveDES), chaveDES)));
        reader.close();
        writer.close();
        writer2.close();
    }
    ...
    public static String desconverterCifra(String texto) {
        String novo = "";
        // Conta até o final da String
        for (int i = 0; i < texto.length(); i++) {
            // Pega o numero do char na tabela ASCII e diminui 3
            int cifra = ((int) texto.charAt(i) - 3);
            // Adiciona a letra na nova string
            novo += (char) cifra;
        }
    ...
    public static byte[] converterDES(String texto, SecretKey chaveDES) {
        try {
            // Cria a cifra
            Cipher cifraDES;
            ...
            String str = new String(textoEncriptado);
            FileWriter writer2 = new FileWriter("des.txt");
            writer2.write(str);
            writer2.close();
    ...
    public static String desconverterDES(String texto, SecretKey chaveDES, byte[] textoEncriptado) {
        try {
            // Cria a cifra
            Cipher cifraDES;
            cifraDES = Cipher.getInstance("DES/ECB/PKCS5Padding");
